...
describe('Shop Component', () => {
  ...
  beforeEach(() => {
    fetch.mockResponseOnce(
      JSON.stringify(dummyRewardsData),
      { status: 200 }
    );
    fetch.mockResponseOnce(
      JSON.stringify(dummyPointsData),
      { status: 200 }
    );
  });
  ...
  test('shows sidebar with correct active rewards', async () => {
    ...
    const title1 = screen.queryByRole('heading', { level: 2, name: dummyRewardsData[1].reward_name }); //disambiguates from shop listing version
  ...
  test('shows only avatars on the avatars panel', async () => {
   ...
   //check existence of all the rewards, only avatars should appear
    ...
    const title1 = screen.queryByRole('heading', { level: 3, name: dummyRewardsData[1].reward_name }); //disambiguates from sidebar version
  ...
  test('equip button is shown and enabled for an owned but inactive reward', async () => {
   ...
   const equipButton = screen.getByTestId(dummyRewardsData[0].reward_name + " Container").querySelector('button');
  ...
  test('equip button is shown but disabled for an owned and active reward', async () => {
   ...
   //validate button attributes
   const equipButton = screen.getByTestId(dummyRewardsData[1].reward_name + " Container").querySelector('button');
    expect(equipButton).toHaveTextContent("Equip");
  ...
  test('buy button is shown and enabled for an unowned, affordable reward', async () => {
   ...
   //validate button attributes
    const buyButton = screen.getByTestId(dummyRewardsData[2].reward_name + " Container").querySelector('button');
  ...
  test('buy button is shown but disabled for an unowned, unaffordable reward', async () => {
   ...
   //validate button attributes
   const buyButton = screen.getByTestId(dummyRewardsData[3].reward_name + " Container").querySelector('button');
    expect(buyButton).toHaveTextContent("Buy");
  ...
  test('correctly handles display when something gets purchased', async () => {
    ...
    expect(window.confirm).toHaveBeenLastCalledWith(`Are you sure you want to purchase ${dummyRewardsData[2].reward_name} for ${dummyRewardsData[2].point_cost} points?`);
